...
function parseSubmissionForm(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (yearMatch || (line.length > 5 && line.length < 50)) {
      ...
      currentCard = { year: yearMatch ? yearMatch[1] : '', card_set: '', card_number: cardNumMatch ? cardNumMatch[1] : '', player_name: '', variety: '' };
      ...
      if (!nameMatch && !yearMatch && line.length > 2) { currentCard.player_name = line; }
    }
  }
  if (currentCard && (currentCard.player_name || currentCard.year)) { cards.push(currentCard); }
  return cards.filter(card => card.player_name || card.year || card.card_set).map(card => ({ year: card.year || '', card_set: card.card_set || 'Unknown', card_number: card.card_number || '', player_name: card.player_name || 'Unknown', variety: card.variety || '' }));
}
