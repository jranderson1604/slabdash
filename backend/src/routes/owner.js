  ...
  try {
    const stats = await db.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM submissions WHERE shipped = false AND (problem_order = false OR problem_order IS NULL) AND (picked_up = false OR picked_up IS NULL)) as active_submissions,
        (SELECT COUNT(*) FROM submissions WHERE shipped = true) as shipped_submissions,
        (SELECT COUNT(*) FROM submissions WHERE grades_ready = true AND shipped = false) as ready_submissions,
        (SELECT COUNT(*) FROM submissions WHERE problem_order = true AND shipped = false) as problem_submissions,
  ...
  try {
    const result = await db.query(`
      SELECT
        ...
        COUNT(DISTINCT CASE WHEN s.shipped = false AND (s.problem_order = false OR s.problem_order IS NULL) THEN s.id END) as active_submissions,
        COUNT(DISTINCT CASE WHEN s.shipped = true THEN s.id END) as shipped_submissions,
        COUNT(DISTINCT CASE WHEN s.grades_ready = true AND s.shipped = false THEN s.id END) as ready_submissions,
        COUNT(DISTINCT CASE WHEN s.problem_order = true AND s.shipped = false THEN s.id END) as problem_submissions,
