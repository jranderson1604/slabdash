...
// Get current company (for frontend)
router.get('/me', authenticate, async (req, res) => {
    try {
        const result = await db.query(
            `SELECT id, name, slug, email, phone, website, logo_url, psa_api_key,
             primary_color, secondary_color, auto_refresh_enabled, auto_refresh_interval_hours,
             email_notifications_enabled, plan, created_at FROM companies WHERE id = $1`,
            [req.companyId]
        );
...
// Update current company (for frontend)
router.patch('/me', authenticate, async (req, res) => {
    try {
        const allowed = ['name', 'email', 'phone', 'website', 'logo_url', 'primary_color', 'secondary_color', 
                         'auto_refresh_enabled', 'auto_refresh_interval_hours', 'email_notifications_enabled', 'psa_api_key'];
